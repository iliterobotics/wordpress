 ...
 * @todo https://github.com/retlehs/ilite/issues/461
 *
 * Rewrite:
 ...
 *
 * @link https://github.com/retlehs/ilite/blob/master/doc/rewrites.md
 */
...
if (stristr($_SERVER['SERVER_SOFTWARE'], 'apache') || stristr($_SERVER['SERVER_SOFTWARE'], 'litespeed') !== false) {
  ...
  function ilite_htaccess_writable() {
    if (!is_writable(get_home_path() . '.htaccess')) {
      if (current_user_can('administrator')) {
        add_action('admin_notices', create_function('', "echo '<div class=\"error\"><p>" . sprintf(__('Please make sure your <a href="%s">.htaccess</a> file is writable ', 'ilite'), admin_url('options-permalink.php')) . "</p></div>';"));
      }
    }
  }

  add_action('admin_init', 'ilite_htaccess_writable');

  function ilite_add_rewrites($content) {
    global $wp_rewrite;
    $ilite_new_non_wp_rules = array(
      'assets/css/(.*)'      => THEME_PATH . '/assets/css/$1',
      'assets/js/(.*)'       => THEME_PATH . '/assets/js/$1',
      'assets/img/(.*)'      => THEME_PATH . '/assets/img/$1',
      'plugins/(.*)'  => RELATIVE_PLUGIN_PATH . '/$1'
    );
    $wp_rewrite->non_wp_rules = array_merge($wp_rewrite->non_wp_rules, $ilite_new_non_wp_rules);
    return $content;
  }

  function ilite_clean_urls($content) {
  ...
  if (!is_multisite() && !is_child_theme() && get_option('permalink_structure')) {
    if (current_theme_supports('rewrite-urls')) {
      add_action('generate_rewrite_rules', 'ilite_add_rewrites');
    }

    if (current_theme_supports('h5bp-htaccess')) {
      add_action('generate_rewrite_rules', 'ilite_add_h5bp_htaccess');
    }
    ...
    if (!is_admin() && current_theme_supports('rewrite-urls')) {
      ...
      add_filters($tags, 'ilite_clean_urls');
    }
  }

  // Add the contents of h5bp-htaccess into the .htaccess file
  function ilite_add_h5bp_htaccess($content) {
    global $wp_rewrite;
